...
- **snmptrap.go**：主程序文件，包含核心逻辑
- **config.json**：默认配置文件
- **build.sh**：构建脚本，支持编译不同架构的可执行文件
- **deploy.sh**：部署脚本，用于一键安装和管理service服务
- **accept/**：接收端测试接收trap消息的python脚本
- **example/**：高兼容性json配置示例
...
SNMPTrap 遵循 SNMPv2c TRAP2 PDU 规范，包含以下变量：
- `.1.3.6.1.6.3.1.1.4.1.0` - SNMPv2-MIB::snmpTrapOID.0（必需的陷阱OID），值为实际的告警或清除OID
- 自定义OID（告警或清除），值为告警或清除消息
